# Exploration-study design and variant comparison setup developed by the author.
        ...
        for map_name, (grid, start, goal) in maps.items():
            ...
            planners = {
                "Failure-Memory Baseline": FailureMemoryDiffusionPlanner(
                    ...
                    lambda_failure=tuned_lambda_failure,
                    seed=seed,
                ),
                "Dead-End Memory": DeadEndMemoryDiffusionPlanner(
                    env,
                    diffusion_model=diffusion_model,
                    horizon=diffusion_horizon,
                    num_candidates=tuned_k,
                ...
                "Diverse Candidates": DiverseCandidateFailureMemoryPlanner(
                    env,
                    diffusion_model=diffusion_model,
                    horizon=diffusion_horizon,
                    num_candidates=tuned_k,
                ...
                "Adaptive Failure": AdaptiveFailureMemoryDiffusionPlanner(
                    env,
                    diffusion_model=diffusion_model,
                    horizon=diffusion_horizon,
                    num_candidates=tuned_k,
                ...
                "Combined Exploration": CombinedExplorationFailureMemoryPlanner(
                    env,
                    diffusion_model=diffusion_model,
                    horizon=diffusion_horizon,
                    num_candidates=tuned_k,
...
def main():
    print("Running exploration planner comparison...")
    df, summary_csv_path, raw_csv_path = run_exploration_comparison()
    print(f"Saved exploration summary to {summary_csv_path}")
    print(f"Saved exploration raw episodes to {raw_csv_path}")
    print("\nExploration Planner Comparison:")
    print(df)
